...
fn ffi_transforms_utf8_text() {
    ...
    let output = unsafe { rtler::rtler_transform_text(input.as_ptr()) };

    assert!(!output.is_null());
    let output_text = unsafe { CStr::from_ptr(output).to_str().unwrap().to_owned() };
    unsafe { rtler::rtler_free_string(output) };
...
fn ffi_returns_null_for_null_input() {
    let output = unsafe { rtler::rtler_transform_text(std::ptr::null()) };
